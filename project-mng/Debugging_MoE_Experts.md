...
4. **Logging Facilities** - Two systems:
   - `LLAMA_LOG_INFO()`, `LLAMA_LOG_DEBUG()`, `LLAMA_LOG_WARN()` from [`src/llama-impl.h`](../src/llama-impl.h) - for core library code (`src/`)
   - `LOG_INF()`, `LOG_DBG()`, `LOG_WRN()` from [`common/log.h`](../common/log.h) - for application code (`tools/`, `examples/`)
